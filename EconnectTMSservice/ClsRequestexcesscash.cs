        ...
        public void Run(string IncomingMessage, string intid, string MessageGUID, string field37, bool pinmessagesent = false)
        {
            ...
            try
            {
                   ...
                   //// strDeviceid = strDeviceid.Substring(0, 15);
                    ...
                    opps.spInsertPOSTransaction(intid, "0000000000000000", "405000", strAmount, "", "", "", field24, "", "", "", strDeviceid, "", "", "", "", "REQUEST EXCESS CASH " + strAgentID, "", "", strAgentID, "", "", "");
                    ...
                    strResponse += "--------------------------------" + "#";
                    strResponse += "   Request for excess Cash     " + "#";
                    strResponse += "--------------------------------" + "#";
                    ...
                    strMsgHead = "A cash excess request of "+ sharedvars.Currencycode +":" + strAmount + " from teller no  " + strAgentID + " at location  " + branchLocation + " has been sent for authorization. ";
                    ...
                    strMsgSysOPMgr = "A cash excess request of " + sharedvars.Currencycode +": " + strAmount + " from teller no  " + strAgentID + " at location  " + branchLocation + " has been sent for authorization. ";
                   ...
                   // 'log the details of teh shortage cash in table tbshortageExcesscash
                    strInMsg = "insert into tbShortageExcessCash (TellerID,TellerName,amount,Branch,TransactionType) values ('" + strAgentID + "','" + TellerName + "','" + strAmount + "','" + branchLocation + "','ExcessCash')";
                    blninmsg = Clogic.RunNonQuery(strInMsg);
            }
            catch (Exception ex)
            {
                EconnectTMSservice.ClsMain.LogErrorMessage_Ver1(ex, "Requestexcesscash", "Requestexcesscash");
            }
        }
    }
}
